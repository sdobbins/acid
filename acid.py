...
def singularize(words):
    # invariants
    is_invariant = (words.find(any_of(English_invariant_words) + "$") > 0) | \
    ...
    is_man = words.endswith("men") & (words.find("(\\b[ao]|abdo|acu|albu|bitu|fora|hy|lu|ra|regi|ru|se|speci|sta)men$") < 0)
    is_oo = np.any([words.endswith(string) for string in ("teeth", "feet", "geese")], axis = 0)
    is_ouse = (words.find("((\\b|book|head)l|(\\b|dor|field|shrew|tit)m)ice$") > 0)
    remove_last2 = (words.find("\\boxen$") > 0)
    is_die = (words.find("\\bdice$") > 0)
    ...
    need_is_latin = (words.find(Latin_es_plurals_string) > 0) & (words.find("((\\b|brown|bull|hard|hook|shovel|arabi|flavi|fura|man|pyra)n|(hep|lac|mal|pen)t)oses$") < 0) & rule_not_found
    ...
    need_is_greek = words.endswith("eis") & (words.find("(\\bl|sens)eis$") < 0) & rule_not_found
    rule_not_found = rule_not_found & np.logical_not(np.any((need_ex, need_ix, need_is_greek), axis = 0))
  
    need_f = (words.find(English_ves_plurals_string) > 0) & rule_not_found
    need_fe = (words.find(English_ves_plurals_e_string) > 0) & rule_not_found
    need_y = words.endswith("ies") & (words.find(English_ie_singulars_string) < 0) & rule_not_found
    ...
    remove_last3 = remove_last3 | ((words.endswith("busses") | \
                                     (words.endswith("zzes") & \
                                                                                (words.find(English_zz_singulars_plurals_string) < 0))) & \
                                                                        rule_not_found)
    rule_not_found = rule_not_found & ~remove_last3
  
    remove_last = remove_last | (((words.find(English_ie_singulars_string) > 0) | \
                                   (words.find(English_oe_singulars_string) > 0) | \
                                   (words.find("[aeiouy][^aeioux]es$") > 0) | \
                                   words.endswith("mmes") | \
                                   (words.find("(([bcdfglprstz][glr])|(l[csv])|(n[cgrs])|(r[cgsv])|(s[c])|(u)|(((\\b|back|belly|head|stomach|tooth)a|ca|mousta|pana|pista)ch)|(rr)|(tt)|(\\b(ba|ca|ge|ha|mo|pa|pi|ta|wa|cha|try|arti|bati|ripo|langou)st))es$") > 0)) & \
                                                                    (words.find(English_s_singulars_plurals_string) < 0) & \
                                                                    (words.find("\\b" + any_of([string + "es" for string in Latin_us_to_i_singulars]) + "$") < 0) & \
                                                                    rule_not_found)
    rule_not_found = rule_not_found & ~remove_last
  
    remove_last2 = remove_last2 | ((words.find("[^e]es$") > 0) & rule_not_found)
...
def is_singular(words):
    is_singular_with_s = (words.find(English_s_singulars_string) > 0) | \
        ((words.find("[^e]iu?s$") > 0) & ~np.any([words.endswith(string) for string in all_is_plurals], axis = 0)) | \
...
def digested_word_dictionary(words):
    ...
    can_be_made_singular = (words.find("\\b" + any_of(English_s_keepers) + "$") < 0)
    ...
    has_keepable_prefix = (words.find("^" + any_of(English_prefix_keepers)) > 0)
    # removes multiple (nested) prefixes
    # excludes a few difficult cases for further processing below
    delete_prefix = ~has_keepable_prefix & (words.find("\\b(((a|de|ex|post|pre|re|semi|un|well)-)|((anti|dis|im[bmp]|hyper|hypo|in|mis|non|over|sub|super|un|under)-?))") > 0) & (words.find("\\b(none($|theless)|im(migra|pov|prop))") < 0)
    ...
    delete_im_prefix = (words.find("\\bim(migra|pov|prop)") > 0)
    ...
    delete_ir_prefix = (words.find("\\birr") > 0) & (words.find("\\birrigat") < 0)
    ...
    delete_ab_prefix = (words.find("\\babnormal") > 0)
    ...
    delete_mal_prefix = (words.find("\\bmal") > 0) & (words.find("\\bmal(ady|ari|ark|e($|s|ness)|efa|efi|evo|ici|ign|ing|l($|[aeiou])|m|($|t[aeiou]))") < 0)
    ...
    if len(subset) != 0:
        # sometimes "lly" -> "ly"
        ly_to_l = (subset.find("(bi|hi|fri|\\bfu)lly$") > 0)
        ...
        ly_keeper_mask = (subset.find("\\b" + any_of(English_ly_keepers) + "$") < 0)
        # e.g. "happily" -> "happy"
    ...
    if len(subset) != 0:
        # e.g. "mobility" -> "mobile"
        bility_to_bile = (subset.find("(\\bla|mo|nu)bility$") > 0)
        ...
        delete_ity = (np.any([subset.endswith(string) for string in ("ality", "icity", "larity")], axis = 0) & ~np.any([subset.endswith(string) for string in ("complicity", "felicity", "quality")], axis = 0)) | (subset.find(any_of([string + "ity" for string in English_al_keepers]) + "$") > 0)
    ...
    subset = words[subset_scope]
    if len(subset) != 0:
        able_keeper_mask = (subset.find("\\b" + any_of(English_able_keepers) + "$") < 0)
        # e.g. "reliable" -> "rely"
        ...
        # e.g. "sharable" -> "share"
        able_to_e = (subset.find(e_rules + "able$") > 0) & able_keeper_mask
    ...
    subset = words[subset_scope]
    if len(subset) != 0:
        ible_keeper_mask = (subset.find("\\b" + any_of(English_ible_keepers) + "$") < 0)
        ...
        # e.g. "sensible" -> "sense"
        ible_to_e = (subset.find(e_rules + "ible$") > 0) & ible_keeper_mask
    ...
    # e.g. "elder" -> "old"
    eld_to_old = (words.find("\\beld(er|est)?$") > 0)
    ...
    istic_to_ism = words.endswith("istic") & (words.find("\\bstatistic") < 0)
    ...
    ist_to_ism = words.endswith("ist") & (words.find("\\b" + any_of(English_ist_keepers) + "$") < 0)
    ...
    if len(subset) != 0:
        ...
        # general rule--remove suffix
        delete_ism = subset.endswith("ism") & (subset.find("\\b" + any_of(English_ism_keepers) + "$") < 0)
    ...
    if len(subset) != 0:
        ...
        ial_to_e = (subset.find("([aeiou][bcs]|[nr]c)ial$") > 0) & (subset.find(any_of(English_ial_keepers) + "$") < 0)
        subset[ial_to_e] = [string[:-3] + "e" for string in subset[ial_to_e]]
        # general "ial" rule--remove suffix
        delete_ial = subset.endswith("ial") & (subset.find(any_of(English_ial_keepers) + "$") < 0)
        ...
        delete_ual = subset.endswith("ual") & (subset.find(any_of(English_ual_keepers) + "$") < 0)
        ...
        # e.g. "global" -> "globe"
        al_to_e = np.any([subset.endswith(string) for string in ("communal", "global", "tribal", "practical", "bridal", "tribunal", "brutal", "ral", "sal", "val")], axis = 0) & (subset.find("(((behavi|doct|elect)o)r|(medie|na|\\bo|flor|prime|\\bri)v)al$") < 0) & (subset.find(any_of(English_al_keepers) + "$") < 0)
        ...
        # general rule--remove suffix
        delete_al = subset.endswith("al") & (subset.find(any_of(English_al_keepers) + "$") < 0)
    ...
    if len(subset) != 0:
        ...
        # e.g. "graphic" -> "graph"
        delete_ic = np.any([subset.endswith(string) for string in ("alphabetic", "graphic", "gymnastic", "istic", "phoric", "xic")], axis = 0) & (subset.find("\\bstatistic") < 0)
    ...
    if len(subset) != 0:
        ...
        delete_ous = (subset.find("((amor|circuit|fever|hazard|joy|nym|ponder|solicit|sulfur|tuber|ulcer|valor|vapor|vermin|viper|vomit|zeal)|((advanta|coura)ge)|((censor|tort)i)|((extra)ne)|((incest)u))ous$") > 0) #*** assume delete all but use ous keepers instead
        subset[delete_ous] = [string[:-3] for string in subset[delete_ous]]
        # e.g. "famous" -> "fame"
        ous_to_e = (subset.find("((fam|nerv|por)|((presti)gi)|((multitu|vicissitu)din)|((auspi|avari|capri|conscien|gra|office|mali|spa|vi)ci)|((raptu)r))ous$") > 0)
    ...
    if len(subset) != 0:
        ...
        # general rule--remove suffix
        delete_ful = subset.endswith("ful") & (subset.find("\\b(aw|grate)ful$") < 0)
    ...
    if len(subset) != 0:
        ...
        # general rule--remove suffix
        delete_less = subset.endswith("less") & (subset.find("(\\b(b|hap|(never|none)the|un)?l)ess$") < 0)
    ...
    ence_to_ent = words.endswith("ence") & (words.find("(\\b(h|sci|sp|th|wh)|[fp]|audi|ess|experi|influ|lic|sent)ence$") < 0) # np.any([words.endswith(string) for string in ("abhorrence", "absence", "accidence", "congruence", "diligence", "evidence", "immanence", "indolence", "inherence", "insistence", "nascence", "opulence", "patience", "permanence", "potence", "presence", "prudence", "quence", "residence", "reticence", "reverence", "salience", "tangence", "transcience", "valence", "violence")], axis = 0)
    ...
    if len(subset) != 0:
        ...
        # e.g. "relative" -> "relate"
        ative_to_ate = subset.endswith("ative") & (subset.find("(\\b[dn]|put)ative$") < 0)
    ...
    if len(subset) != 0:
        ...
        ition_to_ice = (subset.find("practition(er)?s?$") > 0)
    ...
    if len(subset) != 0:
        # "(in)evit(able)" -> "evade"
        evit_to_evade = (subset.find("\\bevit$") > 0)
    ...
    if len(subset) != 0:
        # e.g. "lysis" -> "lyse"
        ysis_to_yse = (subset.find("\\blysis$") > 0)
    ...
    subset = words[subset_scope]
    if len(subset) != 0:
        er_keeper_mask = (subset.find(any_of(English_er_keepers) + "$") < 0)
        # e.g. "controller" -> "control"
        delete_er_letter = ((np.any([subset.endswith(string) for string in flat_concat([English_doubled_consonants_er, "awfuller", "compeller", "controller", "traveller", "quizzer", "frolicker", "mimicker", "mosaicker", "panicker", "picnicker", "politicker", "trafficker", "laughter", "remainder"])], axis = 0) & (subset.find("((([aiu]|\\b([fhjstwy]|bests|dw|kn|kv|qu|sh|sm|sp|sw)e|((\\b|en)r)o)ll)|(\\bodd))er$") < 0)) | (subset.endswith("eer") & (subset.find(any_of(English_eer_keepers) + "$") < 0) & ~np.any([subset.endswith(string) for string in ("decreer", "fleer", "freer", "seer")], axis = 0))) & er_keeper_mask
        subset[delete_er_letter] = [string[:-3] for string in subset[delete_er_letter]]
        # e.g. "carrier" -> "carry"
        ier_to_y = subset.endswith("ier") & (subset.find(any_of(English_ier_keepers) + "$") < 0) & (subset.find("((\\b|water)sk|tax)ier$") < 0)
        subset[ier_to_y] = [string[:-3] + "y" for string in subset[ier_to_y]]
        er_keeper_mask = ~delete_er_letter & (subset.find(any_of(English_er_keepers) + "$") < 0)
        # e.g. "(over)seer" -> "see"
        delete_r = ((subset.find(e_rules + "er$") > 0) | np.any([subset.endswith(string) for string in ("decreer", "fleer", "freer", "seer")], axis = 0)) & er_keeper_mask
    ...
    subset = words[subset_scope]
    if len(subset) != 0:
        est_keeper_mask = (subset.find(any_of(English_est_keepers) + "$") < 0)
        # e.g. "biggest" -> "big"
        delete_est_letter = np.any([subset.endswith(string) for string in flat_concat([English_doubled_consonants_est, "awfullest"])], axis = 0) & (subset.find("(([aiu]ll)|(\\bodd))est$") < 0) & est_keeper_mask
        subset[delete_est_letter] = [string[:-4] for string in subset[delete_est_letter]]
        # e.g. "earliest" -> "early"
        iest_to_y = subset.endswith("iest") & (subset.find(any_of(English_iest_keepers) + "$") < 0)
        subset[iest_to_y] = [string[:-4] + "y" for string in subset[iest_to_y]]
        est_keeper_mask = ~delete_est_letter & (subset.find(any_of(English_est_keepers) + "$") < 0)
        # e.g. "latest" -> "late"
        delete_st = (subset.find(e_rules + "est$") > 0) & est_keeper_mask
    ...
    subset = words[subset_scope]
    if len(subset) != 0:
        ed_keeper_mask = (subset.find(any_of(English_ed_keepers) + "$") < 0)
        # e.g. "centred" -> "center"
        ...
        delete_ed_letter = np.any([subset.endswith(string) for string in flat_concat([English_doubled_consonants_ed, "compelled", "controlled", "travelled", "quizzed", "frolicked", "mimicked", "mosaicked", "panicked", "picnicked", "politicked", "trafficked"])], axis = 0) & (subset.find("(([aiu]|\\b([fhjtwy]|dw|kn|kv|qu|sh|sm|sp|sw)e|((\\b|en)r)o)ll)ed$") < 0) & ed_keeper_mask
        subset[delete_ed_letter] = [string[:-3] for string in subset[delete_ed_letter]]
        # e.g. "tied" -> "tie"
        delete_d = (subset.find("\\b" + any_of(["died", "lied", "tied", "hogtied"]) + "$") > 0)
        subset[delete_d] = [string[:-1] for string in subset[delete_d]]
        # e.g. "carried" -> "carry"
        ied_to_y = subset.endswith("ied") & (subset.find("((\\b|water)sk|tax)ied$") < 0)
        subset[ied_to_y] = [string[:-3] + "y" for string in subset[ied_to_y]]
        ed_keeper_mask = ~delete_ed_letter & (subset.find(any_of(English_ed_keepers) + "$") < 0)
        # e.g. "wasted" -> "waste"
        delete_d = (subset.find(e_rules + "ed$") > 0) & ~np.any([subset.endswith(string) for string in ("synced", "focused")], axis = 0) & ed_keeper_mask
        subset[delete_d] = [string[:-1] for string in subset[delete_d]]
    ...
    subset = words[subset_scope]
    if len(subset) != 0:
        ing_keeper_mask = (subset.find(any_of(English_ing_keepers) + "$") < 0)
        # e.g. "centring" -> "center"
        ...
        delete_ing_letter = np.any([subset.endswith(string) for string in flat_concat([English_doubled_consonants_ing, "compelling", "controlling", "travelling", "quizzing", "frolicking", "mimick", "mosaicking", "panicking", "picnicking", "politicking", "trafficking"])], axis = 0) & (subset.find("(([aiu]|\\b([fhjstwy]|bests|dw|kn|kv|qu|sh|sm|sp|sw)e|((\\b|en)r)o)ll)ing$") < 0) & ing_keeper_mask
        subset[delete_ing_letter] = [string[:-4] for string in subset[delete_ing_letter]]
        ing_keeper_mask = ~delete_ing_letter & (subset.find(any_of(English_ing_keepers) + "$") < 0)
        # e.g. "waving" -> "wave"
        ing_to_e = (subset.find(e_rules + "ing$") > 0) & ~np.any([subset.endswith(string) for string in ("syncing", "focusing")], axis = 0) & ing_keeper_mask
    ...
    if len(subset) != 0:
        ...
        delete_ify = subset.endswith("ify") & (subset.find("(cert|\\bed|gn|mod|myst|rat|spec|test)ify$") < 0)
        subset[delete_ify] = [string[:-3] for string in subset[delete_ify]]
        words[subset_scope] = subset
  
    ity_to_ite = (words.find("\\bunity$") > 0)
    ...
    if len(subset) != 0:
        ...
        # e.g. "cyclize" -> "cycle"
        ize_to_e = ((subset.find(e_rules + "ize$") > 0) | subset.endswith("mobilize")) & (~subset.endswith("tomize") | subset.endswith("epitomize")) & ~np.any([subset.endswith(string) for string in English_ize_keepers], axis = 0)
    ...
    if len(subset) != 0:
        # e.g. "live" -> "life"
        ive_to_ife = (subset.find("\\blive$") > 0)
